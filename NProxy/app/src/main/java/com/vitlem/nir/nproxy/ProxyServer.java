      ...
      switch(mode){
        case START_MODE:
         try{
           ...
           Log.d(TAG,connectionId + " Main thread(client->remote)stopped.");
         }
        break;
        case ACCEPT_MODE:
          try{
            ...
            abort();
            Log.d(TAG,connectionId + " Accept thread(remote->client) stopped");
          }
        break;
        case PIPE_MODE:
         try{
            ...
            abort();
            Log.d(TAG,connectionId + " Support thread(remote->client) stopped");
         }
        break;
        case ABORT_MODE:
        break;
        default:
         Log.i(TAG,connectionId + " Unexpected MODE " + mode);
      }
   }
...
/////////////////
   private void startSession() throws IOException{
     ...
     try{
       ...
       Log.i(TAG,connectionId + " Auth exception", ioe);
       auth = null;
       return;
     }

     if(auth == null){ //Authentication failed
    	 Log.w(TAG,connectionId + " Authentication failed");
        return;
     }
   ...
   private void onConnect(ProxyMessage msg) throws IOException{
      ...
      Log.i(TAG,connectionId + " Connected to " + s.getInetAddress() + ":" +
          s.getPort());
   ...
   private void onBind(ProxyMessage msg) throws IOException{
      ...
      Log.i(TAG,connectionId + " Trying accept on " + ss.getInetAddress() + ":" + 
          ss.getLocalPort());
   ...
   private void onUDP(ProxyMessage msg) throws IOException{
      ...
      Log.i(TAG,connectionId + " Creating UDP relay server for " +  msg.ip +
          ":" + msg.port);
...
//////////////////
   ...
   private void doAccept() throws IOException{
      ...
      Log.i(TAG,connectionId + " Accepted from "+ s.getInetAddress() + ":" + 
          s.getPort());
   ...
   private synchronized void abort(){
      ...
      mode = ABORT_MODE;
      try{
         Log.i(TAG,connectionId + " Closing connection.");
